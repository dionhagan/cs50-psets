...
char* indexes(const char* path)
{
    char* pthhtml = malloc(sizeof(path) + sizeof("index.html\0"));
    char* pthphp = malloc(sizeof(path) + sizeof("index.php\0"));
    
    //create new variable strings
    strcpy(pthhtml, path);
    strcpy(pthphp, path);
    
    //addend file to path string
    strcat(pthhtml, "index.html\0");
    strcat(pthphp, "index.php\0");
    
    //check second if index.html exists
    if(access(pthhtml, F_OK) != -1)
    {
        //free(pth);
        return pthhtml;
    }
    
    //check first if index.php exists
    if (access(pthphp, F_OK) != -1)
    {
        //free(pth);
        return pthphp;
    }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    else
    {
        ...
        *length = 0;
        ...
        while(feof(file) == 0)
        {
            //try to read in complete buffer of bytes
            // fread returns size_t number of blocks read in
            size_t nbytes = BYTES * fread(buffer, sizeof(BYTE), BYTES, file);
            ...
            {
                *content = NULL;
                *content = realloc(*content, *length + nbytes);
                if (*content == NULL)
                {
                    return false;
                }
            }
            memcpy(*content + *length, buffer, nbytes);
            *length += nbytes;
        }
    return true;
    }
...
const char* lookup(const char* path)
{
    ...
    // return appropriate type for extensions
    // if ext is NULL return ext
    if (strcasecmp(ext, ".css\0") == 0)
        return "text/css\0";
    else if (strcasecmp(ext, ".html\0") == 0)
        return "text/html\0";
    else if (strcasecmp(ext, ".gif\0") == 0)
        return "image/gif\0";
    else if (strcasecmp(ext, ".ico\0") == 0)
        return "image/x-icon\0";
    else if (strcasecmp(ext, ".jpeg\0") == 0)
        return "image/jpeg\0";
    else if (strcasecmp(ext, ".js\0") == 0)
        return "text/javascript\0";
    else if (strcasecmp(ext, ".php\0") == 0)
        return "text/x-php\0";
    else if (strcasecmp(ext, ".png\0") == 0)
        return "image.png\0";
    else
        return ext;
}
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    int rtsize = end - begin;
    
    //copy in the first rtsize chars in begin
    char* rtarget = malloc(rtsize * sizeof(char) + 1);
    if (rtarget == NULL)
    {
    ...
    char* qtest = strchr(rtarget, '?');
    if (qtest == NULL)
    {
        strcpy(abs_path, rtarget);
        query = "";
        //printf("qtest is NULL");    
    }
    else
    {
        // calculate abs_path_size via query_size and rtsize
        query = strchr(rtarget, '?') + 1;
        int query_size = strlen(query);
        int abs_path_size = rtsize - query_size - 1;
        
        strncpy(abs_path, rtarget, abs_path_size + 1);
